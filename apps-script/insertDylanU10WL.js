/**
 * populateDylanU10_WL
 * Reads DYLAN_WATCHLIST + U10_rankings → writes DYLAN_U10_WL
 */
...
function populateDylanU10_WL() {
  var ss = SpreadsheetApp.getActiveSpreadsheet();
  var wlSheet  = ss.getSheetByName('DYLAN_WATCHLIST');
  var rkSheet  = ss.getSheetByName('U10_rankings');
  var outSheet = ss.getSheetByName('DYLAN_U10_WL');
  if (!wlSheet)  { Logger.log("Sheet 'DYLAN_WATCHLIST' not found");  return; }
  if (!rkSheet)  { Logger.log("Sheet 'U10_rankings' not found");      return; }
  if (!outSheet) { Logger.log("Sheet 'DYLAN_U10_WL' not found");      return; }
  var ptsMap      = buildPtsMap_U10_WL(rkSheet);
  var tournaments = readTournaments_U10_WL(wlSheet);
  Logger.log('U10 Watchlist tournaments: ' + tournaments.length);
  clearSheet_U10_WL(outSheet);
  for (var i = 0; i < BLOCKS_U10_WL.length; i++) {
    populateBlock_U10_WL(outSheet, BLOCKS_U10_WL[i], tournaments[i] || null, ptsMap);
  }
  Logger.log('Dylan U10 Watchlist: ' + tournaments.length + ' tournament(s) written.');
}

function readTournaments_U10_WL(wlSheet) {
...
function buildPtsMap_U10_WL(rkSheet) {
...
function lookupPts_U10_WL(name, ptsMap) {
  if (!name) return '0';
  return ptsMap[String(name).trim().toLowerCase()] || '0';
}

function ptsAsNumber_U10_WL(name, ptsMap) {
  return parseFloat(lookupPts_U10_WL(name, ptsMap)) || 0;
}

function clearSheet_U10_WL(sheet) {
  var lastRow = sheet.getLastRow();
  if (lastRow > 0) sheet.getRange(1, 1, lastRow, 7).clearContent();
}

function populateBlock_U10_WL(sheet, block, tourn, ptsMap) {
  if (!tourn) return;
  ...
  for (var i = 0; i < maxRows; i++) {
    ...
    colA.push([name]);
    colB.push([name ? lookupPts_U10_WL(name, ptsMap) : '']);
  }
  ...
  for (var i = 0; i < entries.length; i++) {
    ...
    var pts  = ptsAsNumber_U10_WL(name, ptsMap);
  ...
  for (var i = 0; i < draw.length; i++) {
    ...
    colF.push([name]);
    colG.push([name ? lookupPts_U10_WL(name, ptsMap) : '']);
  }
