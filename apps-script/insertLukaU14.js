 ...
 * populateLukaU14()
 *
 * Reads LUKA_TOURNAMENTS + U14_rankings, computes all values,
 * and writes them into LUKA_U14.
 *
...
function populateLukaU14() {
  ...
  for (var i = 0; i < BLOCKS_U14.length; i++) {
    populateRightBlock_U14(u14Sheet, BLOCKS_U14[i], grade5[i]    || null, rankMap);
    populateLeftBlock_U14 (u14Sheet, BLOCKS_U14[i], nonGrade5[i] || null, rankMap);
  }

  Logger.log(
    'Luka 14U - Grade 5: ' + grade5.length + ', Non-Grade 5: ' + nonGrade5.length + ' tournament(s) written.'
  );
}
...
function readTournaments_U14(ltSheet) {
...
// ─── Rankings ─────────────────────────────────────────────────────────────────
function buildRankMap_U14(rkSheet) {
...
function lookupRank_U14(name, rankMap) {
  if (!name) return 'NO RANK';
  return rankMap[String(name).trim().toLowerCase()] || 'NO RANK';
}

function rankAsNumber_U14(name, rankMap) {
  var r = parseInt(lookupRank_U14(name, rankMap), 10);
  return isNaN(r) ? 99999 : r;
}

// ─── Parse "Wed 21/01/2026 22:13" → timestamp for sorting ────────────────────
function parseEntryDate_U14(str) {
  if (!str) return 0;
...
function writeAsText_U14(sheet, startRow, col, values) {
...
function populateRightBlock_U14(sheet, block, tourn, rankMap) {
  if (!tourn) return;
  ...
  writeAsText_U14(sheet, firstData, 10, colJ);                   // J: entry date (plain text)
  sheet.getRange(firstData, 12, maxRows, 1).setValues(colL); // L: entry order

  // Cols M/N: sorted by entry date ascending
  var sorted = entries.slice().sort(function(a, b) {
    return parseEntryDate_U14(a.dateStr) - parseEntryDate_U14(b.dateStr);
  });
  ...
  for (var i = 0; i < maxRows; i++) {
    ...
    colM.push([name]);
    colN.push([date]);
    colO.push([name ? lookupRank_U14(name, rankMap) : '']);
  }
  sheet.getRange(firstData, 13, maxRows, 1).setValues(colM); // M
  writeAsText_U14(sheet, firstData, 14, colN);                   // N: date (plain text)
  ...
  topEntries.sort(function(a, b) {
    return rankAsNumber_U14(a.name, rankMap) - rankAsNumber_U14(b.name, rankMap);
  });
  ...
  for (var i = 0; i < topEntries.length; i++) {
    ...
    colQ.push([name]);
    colR.push([lookupRank_U14(name, rankMap)]);
  }
...
function populateLeftBlock_U14(sheet, block, tourn, rankMap) {
  if (!tourn) return;
  ...
  for (var i = 0; i < maxRows; i++) {
    ...
    colA.push([name]);
    colB.push([name ? lookupRank_U14(name, rankMap) : '']);
  }
  ...
  for (var i = 0; i < entries.length; i++) {
    ...
    var r    = rankAsNumber_U14(name, rankMap);
    if (r < 99999) {
  ...
  for (var i = 0; i < drawSize; i++) {
    ...
    colF.push([name]);
    colG.push([name ? lookupRank_U14(name, rankMap) : '']);
  }
  if (drawSize > 0) {
    sheet.getRange(firstData, 6, drawSize, 1).setValues(colF);
    sheet.getRange(firstData, 7, drawSize, 1).setValues(colG);
  }
}