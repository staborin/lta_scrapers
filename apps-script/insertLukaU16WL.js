// ═══════════════════════════════════════════════════════════════════════════════
// populateLukaU16_WL
// Reads LUKA_WATCHLIST + U16_rankings → writes LUKA_U16_WL
// All watchlist = non-Grade 5, left block only (cols A-G)
// Rankings: col A = rank, col B = name (ascending = better)
// ═══════════════════════════════════════════════════════════════════════════════
...
function populateLukaU16_WL() {
  var ss = SpreadsheetApp.getActiveSpreadsheet();

  var wlSheet  = ss.getSheetByName('LUKA_WATCHLIST');
  var rkSheet  = ss.getSheetByName('U16_rankings');
  var outSheet = ss.getSheetByName('LUKA_U16_WL');

  if (!wlSheet)  { Logger.log("Sheet 'LUKA_WATCHLIST' not found");  return; }
  if (!rkSheet)  { Logger.log("Sheet 'U16_rankings' not found");     return; }
  if (!outSheet) { Logger.log("Sheet 'LUKA_U16_WL' not found");      return; }

  var rankMap     = buildRankMap_U16_WL(rkSheet);
  var tournaments = readTournaments_U16_WL(wlSheet);

  Logger.log('U16 Watchlist tournaments: ' + tournaments.length);

  clearSheet_U16_WL(outSheet);

  for (var i = 0; i < BLOCKS_U16_WL.length; i++) {
    populateBlock_U16_WL(outSheet, BLOCKS_U16_WL[i], tournaments[i] || null, rankMap);
  }

  Logger.log('Luka U16 Watchlist: ' + tournaments.length + ' tournament(s) written.');
}

function readTournaments_U16_WL(wlSheet) {
...
function buildRankMap_U16_WL(rkSheet) {
...
function lookupRank_U16_WL(name, rankMap) {
  if (!name) return 'NO RANK';
  return rankMap[String(name).trim().toLowerCase()] || 'NO RANK';
}

function rankAsNumber_U16_WL(name, rankMap) {
  var r = parseInt(lookupRank_U16_WL(name, rankMap), 10);
  return isNaN(r) ? 99999 : r;
}

function clearSheet_U16_WL(sheet) {
  var lastRow = sheet.getLastRow();
  if (lastRow > 0) sheet.getRange(1, 1, lastRow, 7).clearContent();
}

function populateBlock_U16_WL(sheet, block, tourn, rankMap) {
  if (!tourn) return;
  ...
  for (var i = 0; i < maxRows; i++) {
    ...
    colA.push([name]);
    colB.push([name ? lookupRank_U16_WL(name, rankMap) : '']);
  }
  ...
  for (var i = 0; i < entries.length; i++) {
    ...
    var r    = rankAsNumber_U16_WL(name, rankMap);
    if (r < 99999) ranked.push({ name: name, rank: r });
  ...
  for (var i = 0; i < draw.length; i++) {
    ...
    colF.push([name]);
    colG.push([name ? lookupRank_U16_WL(name, rankMap) : '']);
  }
