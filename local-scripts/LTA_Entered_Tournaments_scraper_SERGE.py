...
OUTPUT_SHEET = "SERGE_TOURNAMENTS"

DEFAULT_EVENT_TYPE = "MS"
...
def write_wide_to_sheet_via_webapp(tournaments, sheet_tab, clear_first, logger: logging.Logger):
    ...
    payload = {"sheet": sheet_tab, "clearFirst": bool(clear_first), "rows": grid}
    logger.info(f"Posting wide-format grid ({total_rows} rows x {total_cols} cols) to Google Sheet tab '{sheet_tab}'")
...
def login_if_needed(page):
    username = os.environ.get("SERGE_LTA_USERNAME", "").strip()
    password = os.environ.get("SERGE_LTA_PASSWORD", "").strip()

    if not username or not password:
        print("\n⏸️  Env vars not set: SERGE_LTA_USERNAME / SERGE_LTA_PASSWORD")
    ...
    try:
        ...
        for i in range(n):
            ...
            tournaments.append({
                "name": name,
                "date": date_str,
                "event": DEFAULT_EVENT_TYPE,   # ← HARD CODED "MS"
                "link": href,
                "status": None,
                "grade": None,
                "draw_size": None,
...
def enrich_tournament_with_my_entry_event(page, tournament):
    """
    For SERGE scraper:
    Event starts with DEFAULT_EVENT_TYPE ("MS") but may have a suffix (e.g. "MS 45+").
    Opens "My Entry" to read the actual full event text.
    Optionally fills missing date from Fact Sheet.
    """
    ...
    try:
        ...
        # Try to read actual event from "My Entry" to capture variants like "MS 45+"
        event_code = DEFAULT_EVENT_TYPE  # fallback
        try:
            my_entry = page.get_by_role("link", name=re.compile(r"^\s*My Entry\s*$", re.I)).first
            if my_entry.count() > 0:
                my_entry.click()
                page.wait_for_load_state("domcontentloaded")
                page.wait_for_timeout(800)
                ...
                if dt_events.count() > 0:
                    ...
                    # Find all MS* codes (e.g. "MS", "MS 45+", "MS 35+")
                    ms_codes = [e.strip() for e in re.findall(r"\bMS\b[^\n,;]*", event_text)]
                    ms_codes = [e for e in ms_codes if e]
                    if ms_codes:
                        event_code = ms_codes[0]
                        print(f"  ✓ Event read from My Entry: {event_code}")
                    else:
                        print(f"  ⚠️  No MS* event found in My Entry — falling back to '{DEFAULT_EVENT_TYPE}'")
        except Exception as e:
            print(f"  ⚠️  Could not read My Entry event: {e} — falling back to '{DEFAULT_EVENT_TYPE}'")

        tournament["event"] = event_code
        tournament["events"] = [event_code]
        print(f"  ✓ Event set to: {tournament['event']}")

        # If date missing from card, fetch from Fact Sheet
        if not tournament.get("date"):
            try:
                ...
                dt_start = page.locator(
                    "dt.list__label",
                    has_text=re.compile(r"^\s*Start date\s*$", re.I)
                ).first

                if dt_start.count() > 0:
                    dd_start = dt_start.locator("xpath=following-sibling::dd[1]").first
                    start_txt = (dd_start.inner_text() or "").strip()
                    m = re.search(r"\b\d{2}/\d{2}/\d{4}\b", start_txt)

                    if m:
                        tournament["date"] = m.group(0)
                        print(f"  ✓ Date from Fact Sheet: {tournament['date']}")

            except Exception:
                pass

    except Exception as e:
        print(f"  ⚠️  Could not enrich tournament: {e}")
...
def scrape_tournament_details(page, tournament, scrape_entries=True):
    ...
    try:
        ...
        # Find event button: match full event text, or fall back to startswith MS
        all_buttons = page.query_selector_all("button")
        event_button = None
        for btn in all_buttons:
            try:
                btn_text = btn.inner_text().strip()
                if btn_text == tournament["event"]:
                    event_button = btn
                    break
            except Exception:
                continue
        # Fallback: if no exact match, accept any button starting with "MS"
        if not event_button:
            for btn in all_buttons:
                try:
                    btn_text = btn.inner_text().strip()
                    if btn_text.upper().startswith("MS"):
                        event_button = btn
                        tournament["event"] = btn_text  # update to actual value on page
                        tournament["events"] = [btn_text]
                        print(f"  ↳ Fallback: matched button '{btn_text}'")
                        break
                except Exception:
                    continue
...
def main():
    ...
    with sync_playwright() as p:
        ...
        # Process each tournament, splitting multi-event entries into separate tournament dicts
